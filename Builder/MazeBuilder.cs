        ...
        public IMazeBuilder AddEnemies()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                do
                {
                    x = 1 + _random.Next(_maze.Width - 2);
                    y = 1 + _random.Next(_maze.Height - 2);
                } while (_maze.MazeBuffer[x, y] == Constants.Wall && _maze.GetEnemyfromPosition(x, y) == null);

                Enemy enemy = ItemFactory.GenerateEnemies();
                enemy.Position = new System.Drawing.Point(x, y);
